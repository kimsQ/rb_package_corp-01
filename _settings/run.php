...
if(!is_dir($filesFolder)){
  mkdir($filesFolder,0707);
  @chmod($filesFolder,0707);
}
...
for($i = 1; $i < $RSS['count']; $i++)
{
  ...
  if ($upload)
  {
    ...
    foreach($upfiles as $val)
    {
      ...
      $QVAL = "'$up_gid','0','','$site','$mbruid','$up_type','$up_fileExt','$up_fserver','$up_url','$up_folder','$up_name','$up_tmpname','$up_size','$up_width','$up_height','$up_caption','$up_down','$up_src','$d_regis','','$up_sync'";
      getDbInsert($table['s_upload'],$QKEY,$QVAL);
